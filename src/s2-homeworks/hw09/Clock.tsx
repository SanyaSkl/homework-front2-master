...
function Clock() {
    ...
    const getClockString = (num: number) => num < 10 ? '0' + num : num
    ...
    const stringTime = `${getClockString(date.getHours())}:${getClockString(date.getMinutes())}:${getClockString(date.getSeconds())}` ||
        <br/> // часы24:минуты:секунды (01:02:03)/(23:02:03)/(24:00:00)/(00:00:01) // пишут студенты
    const stringDate = `${getClockString(date.getDate())}.${getClockString(date.getMonth() + 1)}.${getClockString(date.getFullYear())}` ||
